...
int main() {
  ...
  auto mpsc_test = [](const size_t buffer_size) {
    ...
    for (int thread_count = 1; thread_count <= 200;
      ...
      auto mpsc_time = LOGGER_TIME_CODE({ umq_mpsc(buffer_size, thread_count, publish_count, 16); });
      auto fifo_time = LOGGER_TIME_CODE({ fifo_mpsc(buffer_size, thread_count, publish_count, 16); });
